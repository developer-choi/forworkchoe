import {ApiRequestError, ApiResponseError, MismatchedApiResponseError} from '@/utils/error/class/warning/fetch.ts';
import {NotAuthenticatedError} from '@/utils/error/class/info/auth.ts';
import {type ConvertableQuery, stringifyQuery} from '@/utils/query-string/convert.ts';
...
async function extractResponseData<D>(response: Response, options: BaseApiRequest): Promise<D> {
  ...
  try {
    ...
    throw new MismatchedApiResponseError(options, response, error);
  }
