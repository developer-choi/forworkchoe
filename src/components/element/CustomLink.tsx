...
import React, {CSSProperties, PropsWithChildren, Suspense} from 'react';
...
function InnerLink({ className, href, enableActive, ...rest }: CustomLinkProps) {
  const activeClass = useCheckHrefIsActive(href, enableActive);
  return <Link href={href} className={classNames(className, activeClass)} {...rest} />;
}

/**
 * href는 "?query=value"와 "/current/path?query=value" 모두 지원함.
 */
function useCheckHrefIsActive(href: LinkProps['href'], enableActive: CustomLinkProps['enableActive']): string | undefined {
  ...
  if (isActive) {
    return enableActive?.className as string;
  } else {
    return undefined;
  }
}
